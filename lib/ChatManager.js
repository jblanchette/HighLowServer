...
ChatManager.prototype.leaveAllRooms = function (member) {
...
ChatManager.prototype.setupNamespace = function () {
  ...
  namespace.addHandler({
    ...
    fn: function (socket, user, data) {
      ...
      var result = self.joinRoom(user, data);

      if (result) {
        socket.join(result.roomId);

        // todo: get member info from auth  / user service
        socket.to(result.roomId).emit(ChatSchema.memberJoined, user);
      }
    }
  });
  ...
  namespace.addHandler({
    ...
    fn: function (socket, user, data) {
      ...
      if (joinedRoom) {
        ...
        socket.to(joinedRoom.id).emit(ChatSchema.userJoined, joinedRoom);
      } else {
