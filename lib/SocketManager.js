...
SocketManager.prototype.createNamespace = function (options) {
	...
	return {
		...
		create: function () {
			...
			console.log("Created socket namespace: " + namespace.name);
			console.log("Handlers: ", _.pluck(namespace.handlers, "key"));
			console.log("==========================================");
			return namespace;
		},
		addHandler: function (handler) {
			if (!handler || !handler.key) {
				console.error("**ERROR** Passed a socket handler without a key", handler);
				return;
			}
