...
GameManager.prototype.getNamespace = function () {
	var self = this;
	var options = {
		channelName: "gameList",
		handlers: {}
	};

	var handlers = options.handlers;
	handlers[GameListSchema.gameList] = function (socket, data) {
		console.log("Running game list handler: ", self.readGameList());
		socket.emit(GameListSchema.gameList, self.readGameList());
	};

	handlers[GameListSchema.joinGame] = function (socket, data) {
		var gameId = data.gameId;
		var member = data.member; // todo: this should be an ID and a lookup for the member

		var game = self.joinGame(gameId, member);
